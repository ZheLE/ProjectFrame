...
import com.project.frame.common.ImageLoaderScroll;
 ...
 * 滑动时 取消所有加载项
 */
public class ScrollLoadLruCacheAdapter extends RecyclerView.Adapter<ScrollLoadLruCacheAdapter.ViewHolder> implements View.OnClickListener {

    private Context mContext;
    private List<VenuesInfo> mDatas;
    private ImageLoaderScroll mImageLoaderScroll;
    ...
    public ScrollLoadLruCacheAdapter(Context context, List<VenuesInfo> datas){
        this.mContext = context;
        this.mDatas = datas;

        mInflater = LayoutInflater.from(mContext);

        if(mImageLoaderScroll == null){
            mImageLoaderScroll = new ImageLoaderScroll();
        }
    }
...
//        mImageLoader.showImageByThread(viewHolder.img, imgUrl); //加载图片
        mImageLoaderScroll.showImageByAsyncTask(viewHolder.img, imgUrl);
